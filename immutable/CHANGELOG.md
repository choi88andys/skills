...
- **`requirement_contract:` config block** (optional; no `version:` bump) — `enforcement` (`optional` | `required`), `tracker`, `repo`, `fetch_command`. A malformed block is fatal to the validator, never a silent "off".
