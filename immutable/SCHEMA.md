...
### v0.11 additions — `requirement_contract` (optional)

A repo whose pitches are authored against tracker tickets declares the contract here. The block is **opt-in**: absent, `/immutable:prd` never mentions tickets and the validator checks nothing new. No `version:` bump — it is an optional key on a v3 (or v2) config.

```yaml
# Requirement contract (v0.11+, optional). See "Requirement contract" below.
requirement_contract:
  # optional — a pitch MAY cite a ticket; required — every pitch cites one or
  # records `references.ticket_exemption` with a one-line reason.
  enforcement: optional
  # Label copied into each pitch's `references.tickets[].tracker`.
  tracker: github
  ...
  # GitHub adapter (`gh api graphql`).
  # fetch_command: "python3 tools/jira_ticket.py {id}"
```

The vocabulary the contract needs — which ticket headings bind, the three admissible deviation categories, the disagreement section's labels — lives in the **profile**, not here (see "Requirement contract" below).
...
references:
  ...
  tickets:                    # pitch, v0.11+: the tracker tickets this pitch is authored against
    - tracker: github
      repo: my-org/task-tracker
      id: "3755"
...
9. **Requirement contract (v0.11+)** — three checks, **none of which applies to a `deprecated: true` doc** (v0.12.1). A dead doc in an append-only repo has no remedy: it cannot be edited, and superseding it would revive it — so a check tightened after it was written would fail it forever. The count of frozen docs goes to stderr, never nowhere. An **active** doc with the same defect still fails, because its remedy is a new pitch; `coverage` and `drift` read active docs only, so nothing downstream loses a check. For an active doc, the first check is a frontmatter-shape check and always runs; the other two are no-ops for a repo without a `requirement_contract:` config block:
...
sections:
  ...
  # …
  # v0.11+: rendered only when the pitch deviates from a binding ticket item.
...
# Requirement contract vocabulary (v0.11+, profile_schema 3). Inert unless
# config.yml declares `requirement_contract:`. See "Requirement contract".
requirement_contract:
...
## Requirement contract (v0.11.0, profile_schema 3)

A pitch can be authored against a tracker ticket whose designated sections **bind** it. In the pilot the ticket is a GitHub Epic and the binding sections are 「완료 조건」 and 「QA 통과 리스트」; the plugin knows neither name — both come from the profile — and obtains the ticket through an adapter, so a team on another tracker plugs in one command and changes nothing else.

### What the plugin defines, and nothing more

Three shapes, all owned here; everything around them belongs to the consuming repos:
...
One `### ` entry per contested item; four labelled bullets whose labels come from `profile.requirement_contract.disagreement.fields`:

```markdown
## 요구사항 이견

### github my-org/task-tracker#3755 · 완료 조건 · 적립 7

- **원문** <the ticket's sentence, verbatim>
- **정정** <the sentence this pitch follows>
...
### Profile field migration (v0.5.7+, universal diff in v0.5.8+)

Plugin updates may add new fields to `profile_schema`. v0.5.6 introduced `profile_schema: 2` (added `sections[].max_items`, `sections[].structure`, top-level `anti_monolith` block, `gate.criteria[concern_scope]`, `personas[quality_auditor]`, `vague_words` list, `adr.anti_monolith`). Teams who ran `/immutable:migrate` before this update have a frozen v1 team profile that does not benefit from the new defaults — until they re-run `/immutable:migrate` v0.5.7+.

`/immutable:migrate` v0.5.7+ has two responsibilities:

1. **Config migration** (`v2 → v3`) — unchanged from prior releases.
2. **Profile field migration** — reads the team's `profile_schema:`, compares the team profile's structure to the bundled default's structure, and inserts only **missing** fields/entries. Never modifies values the team already has. Bumps `profile_schema:` to match the bundled version on success.

#### v0.5.7 vs v0.5.8 implementation

| | v0.5.7 (recipes) | v0.5.8+ (universal diff) |
|---|---|---|
| Approach | Per-version migration recipe (e.g., "v1→v2 adds these specific fields") | Compare team profile structure vs bundled default; mark every additive difference |
| New plugin field | Requires SKILL.md update (recipe entry) | Picked up automatically — no skill update needed |
| Risk of omission | High (v0.5.7 recipe missed `sections[user_stories].structure`) | None — diff is exhaustive by construction |
| Override preservation | Explicit per-recipe rules | Single algorithmic guarantee: scalars and anonymous lists never modified; id-keyed entries recursively walked but existing fields preserved |

The universal diff (v0.5.8+) is the source of truth. Per-version recipes from v0.5.7 are kept only as descriptive worked examples in the SKILL.md.

#### Override preservation guarantees

The algorithm enforces these invariants:

- Scalars in the team profile are never modified.
- Existing id-keyed entries (matched by `id` or `key`) are recursively walked but never replaced.
