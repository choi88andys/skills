...
- **Any new field in `immutable/examples/_profiles/default-*.yml` bumps `profile_schema` in both files.** `/immutable:migrate`'s universal diff runs only when the team profile's `profile_schema` is *below* the bundled one; a field added without a bump is invisible to every team that already migrated (measured 2026-09-09: `deferral_patterns` added under schema 3 reached no team profile until copied by hand). Additive is not exempt — the bump is the signal, not a statement about breakage.
