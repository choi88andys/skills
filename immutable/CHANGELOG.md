...
The pilot that drove this has UX-authored GitHub Epics whose acceptance-criteria and QA-checklist sections bind the pitch that implements them; the rest of the ticket — Notion links, Figma frames — is input, not verdict. Three things had to become true: the pitch author reads exactly those sections, item by item; a ticket item that cannot be followed as written becomes a *recorded* deviation rather than a silent one; and a ticket edited after the pitch was written is noticed by something other than luck.

The design went through one reversal worth recording. The first plan quoted the binding sentences into the pitch so CI could compare the quote with the live ticket. That would have recreated exactly the two-sources-of-truth problem that made the ticket's own requirement section links-only. A tracker that keeps edit history makes the quote redundant: the ticket's identity plus the version coordinate the author read reconstructs the text, and GitHub's edit log returns the full body after every edit (verified 2026-09-08 on a six-edit Epic with `gh api graphql`). So the pitch records `references.tickets[]` — tracker, id, version, no text — and `drift` re-reads the ticket at that version to show which items moved.

Nothing tracker-specific is baked in. Which headings bind, the three admissible deviation categories and the disagreement vocabulary are profile data; how a ticket is obtained is an adapter command the consuming repo names; the version coordinate is an opaque string the plugin only ever tests for equality. Deliberately outside the plugin: any clock (business-day deadlines and holiday calendars live with the tracker), any write to the tracker (correction requests are handed to the author to post), and any coverage check against sources the ticket does not name — a topic the ticket never raised is not a defect, the pitch fills it.

### Added

- **`scripts/requirement_contract.py`** — stdlib-only. `parse` turns a ticket body into binding items: whole-line `**bold**` group labels between headings (the pilot template's shape, which a heading-only split silently loses), deeper headings as groups, task-list / plain / ordered items, continuation lines, struck items, HTML comments blanked with stable line numbers, fenced code ignored, a forgiving heading match. Everything tolerated leaves a `warnings` entry; a missing or empty binding section is a named error with exit 1 and the JSON still emitted, because most older tickets in the wild are malformed and the skill must render the reason rather than guess. `fetch` obtains the ticket through the built-in GitHub adapter (`gh api graphql`, edit history included) or a `--fetch-command` adapter (shlex-split, placeholders substituted per argv element, never a shell). `drift` compares a recorded version with the live one and, when history holds the recorded body, diffs the binding items as multisets — exit 0 when nothing binding moved (an edit confined to non-binding text included), 1 when an item was added or removed or the change could not be examined. `requirement_contract.test.sh` pins 21 hermetic cases, `fetch` and `drift` through a stub `gh` and a custom adapter.
- **`requirement_contract:` config block** (optional; no `version:` bump) — `enforcement` (`optional` | `required`), `tracker`, `repo`, `since`, `fetch_command`. `since` is the adoption date: under `required`, only pitches dated on or after it must cite a ticket, because the ones written before the contract existed are append-only and cannot be edited into compliance — the first pilot run showed six pitches inside the `--strict-since` window failing retroactively without it. A malformed block, `since` included, is fatal to the validator, never a silent "off".
- **Profile schema 3** — `sections[id=requirement_disagreement]` (`required: false`, rendered only on a deviation) and a top-level `requirement_contract` block: `ticket_sections[]`, `categories[]` (not implementable / self-contradiction / conflict with a settled requirement), `disagreement.fields`, the provisional / terminal outcome tokens, and `disagreement.deferral_patterns[]` — the phrases that make a correction a deferral. Identical structure in default-ko and default-en, as the universal-diff migration requires.
- **A correction is a decision.** The first real pitch through the contract wrote a correction that read "the threshold is to be confirmed with the other team" — a question, and a question cannot be confirmed by silence, which is the protocol's default path. Stage 1.6 now requires the correction to be the sentence that becomes the requirement if the ticket stays silent (the developer decides the open value in the interview), forbids pending language about the contested item anywhere in the body, and the validator flags a correction matching a deferral pattern. `requirement_contract.response_window` (free text, e.g. "2영업일") is rendered into the request; the plugin keeps no calendar.
- **The correction request is a file.** Stage 1.6 writes it to `.claude/immutable/contract/<slug>-<tracker>-<id>.md` in ticket register — request date, response window, original / correction / reason per item, no pitch-internal state — so it can be posted as-is instead of transcribed from a chat transcript or, worse, pasted from the pitch's disagreement section.
- **Accounting by ledger, over the set.** The first seven-pitch run under the per-pitch accounting rule produced 62 out-of-scope bullets of which 48 pointed at sibling pitches, a badge pitch with 6 normatives and 8 handoffs, handoffs written before their target existed that became false when it did, and no place to record that a QA item's literal copy was reflected in substance but delegated to Figma. All four are one defect: the Epic binds the *set* of pitches, and the rule reconciled one pitch at a time. Each pitch now declares in `references.tickets[].covers` the groups or in-group items it reflects and in `delegates` what it reflects in substance while the literal is owned elsewhere; `requirement_contract.py coverage` reconciles every pitch citing the ticket against the live items (`uncovered` / `overlaps` / `stale` / `shared`, exit 0 only when every item is claimed exactly once or shared by agreement); the validator checks the ledger's shape; a pitch's 범위 제외 carries at most one sibling bullet, without filenames. Group labels are the ticket's, verbatim, so a renamed group surfaces as `stale` — the same drift the `drift` command reports.
- **One item coordinate.** `<group> <n>` means the same thing everywhere: n is the item's position within its group. The parser now emits `in_group` beside the section-wide `ordinal` (kept as a sort key), `coverage` rows name items by `in_group`, and the prd skill fills `{ordinal}` in judgement rows, the correction request and the disagreement header from it. Found by the second seven-pitch run: its correction request cited 「쿠폰·할인 적용 주문의 적립」 11 for a five-item group — the section-wide number the skill had pointed at, while the same author's ledger, written minutes later, already counted within the group. A reader of the ticket cannot resolve 11 in a group of five.
- **Value propagation and supersede completeness.** A finding that turns on a literal value sweeps the other binding items for the same value (the first run contested the coupon-case "1,000원" and left the general "1,000원 이상" minimum firm). A Stage 5 handoff must name an *existing* active pitch; a target that does not exist yet is a recorded gap listed in the handoff, and bound to the same PR when the superseded pitch used to state it. For `update` intent, Stage 6 refuses to deprecate the old pitch while something it bound has no active statement — carried, handed off, or closed by a same-PR sibling that also supersedes it (fan-out).
- **Pitch frontmatter** — `references.tickets[]` (identity + version coordinate, `read_at`, `url`) and `references.ticket_exemption` (the one-line reason a pitch cites no ticket under `required`).
- **`/immutable:prd` Stage 1.5.1 and Stage 1.6** — ticket intake (each parser exit code has a scripted path) and requirement judgement (every binding item classified with the evidence it must cite; "a better way", "lacks detail" and "the ticket never raised this" are explicitly not findings; the user accepts or rejects each finding; accepted ones become disagreement entries with a drafted correction, and the author receives the correction request to post). Stage 2 puts bound items first among recommended-answer sources; Stage 5 refuses a bound pitch that leaves a binding item unaccounted for, exactly as it refuses an unresolved tag; Stage 6 writes the ticket record and, only when needed, the disagreement section; Hard Prohibitions 11–13 (never write to the tracker; never an entry outside the three categories, with a field missing, or with a terminal outcome; never treat an unraised topic as a defect). Seventeen `prd.contract.*` catalog keys in ko and en at parity.
- **Validator invariant 9** — ticket-record shape (always on); ticket presence under `required` (`--strict-body`, within the `--strict-since` scope); and the **disagreement merge gate**: every entry carries its four bullets and an outcome beginning with a terminal token. A provisional outcome is a violation, which is what keeps a pitch PR open until its correction request is settled. A profile without the vocabulary skips the gate with a stderr warning, never silently. `validate_docs_requirement_contract.test.sh`: 8 cases.
- **SCHEMA.md "Requirement contract"** — the three shapes the plugin owns (ticket input, parsed contract, pitch record), the authoring flow, the disagreement section's format and why it stays after resolution, and how consumers wire `drift` (a spec repo's PR check; a tracker-side bot that reads a registration comment left on the ticket at merge). **README** section. **Starters**: every pitch-authoring config carries the block commented out; every pitch TEMPLATE shows the ticket record and the optional section.

### Fixed

- **The plugin registered zero skills when loaded with `claude --plugin-dir`.** This plugin keeps its skills at the plugin root rather than under `skills/`, so the list must be enumerated in a manifest — and it was enumerated only in the marketplace file. A marketplace install worked; every other load path (`--plugin-dir`, a `.zip`) read `plugin.json` alone, found no `skills`, and loaded nothing. Worse, a `--plugin-dir` plugin with the same name shadows the installed one, so the session lost `/immutable:*` entirely instead of falling back to the installed version. `claude plugin validate` passed in that state. The defect predates this release (0.10.1 has the same shape); the first pre-release dogfood run through `--plugin-dir` is what hit it. `plugin.json` now carries the `skills` array, and the new `scripts/plugin_manifest.test.sh` checks the packaging invariants nothing else checked — both manifests' skill lists against the directories on disk, description and keyword mirroring, semver, strings-catalog parity — every one of which has shipped as a bug before.

- **A date-shaped but impossible config value crashed the validator.** `strict_body_since: 2026-13-01` (unquoted) is built by PyYAML as a date, and `datetime` raises `ValueError` inside the parser — which `load_config` did not catch, so the run ended in a traceback rather than the "not a real calendar date" error the CLI flag produces. Caught now, for `strict_body_since` and the new `requirement_contract.since` alike.

### Changed

- **`--strict-body` gains one rule** for repos that opt in and none for the others. The starter workflows still pin `immutable--v0.8.0` and are unaffected.
- **`profile_schema` is 3** in both bundled defaults. `/immutable:migrate`'s universal diff picks the additions up; until a team re-runs it, the skills read the two new fields from the bundled default with the usual source annotation.

### Backward compatibility

- **Opt-in throughout.** No `config.yml` version bump. A repo without `requirement_contract:` runs the unchanged six stages, the validator emits no new violation, and the parser is never invoked.
- **`profile_schema` 2 → 3 is additive.** A v2 team profile keeps working; both new fields are inert unless the config opts in, and the Stage 1.bis mismatch warning recommends `/immutable:migrate` as before.
- **One new runtime dependency, on the contract path only**: the `gh` CLI for the built-in adapter. The parser and `drift` themselves need python3 alone (no PyYAML).
- **Not shipped**: any workflow that runs `drift`. Where and when it runs is the consuming repo's decision; SCHEMA.md describes the two shapes the pilot uses.
...
The fix ships as the plugin's first hook. `hooks/hooks.json` registers a PostToolUse hook on the Read tool; when the target is a `deprecated: true` document inside an immutable SDD repo (any file with a `.immutable-prd/` ancestor), the hook replaces the tool output so the body never reaches the model — a banner names the superseding file(s) instead, and the frontmatter stays visible in full. Withholding, deliberately, rather than the two cheaper designs: a warning (`additionalContext`) is delivered deterministically but heeded probabilistically, and `deny` breaks reads that are legitimate (audits, walking a supersede chain). Withholding does not depend on being heeded. The frontmatter passes through so the one sanctioned in-place edit — flipping `deprecated` — always has its exact target text, and raw-body access stays available through Bash (`cat`, `git show`) as the deliberate audit path. One shape detail is load-bearing: the emitted `updatedToolOutput` must mirror Read's `tool_response` **object** exactly, because a plain string is a silent no-op — the hook fires, emits valid JSON, exits 0, the harness drops it, and the model reads the real body with zero error on any stream. That is the same invisible-failure class as a gate that looks installed and never runs; a regression test pins the shape so nobody simplifies it back.

The same release closes the opposite failure direction. The flow skills picked "active" pitches with a line grep (`design` §1.2's `grep -L '^deprecated: true'`) or unspecified prose scans (`prd` §1.2.4, `plan-review-ceo` Phase 0.1), and the grep is wrong in both directions on real YAML: `deprecated: True` and `deprecated:  true` are live deprecations it reads as ACTIVE — a dead spec offered for implementation — while a fenced example line inside a body reads as DEPRECATED — a live pitch silently dropped from every picker. A 23-repo / 1,017-doc scan (2026-07-16) found zero live divergence, so the grep was *latently* broken rather than the incident's cause — but latent is one authored document away from live. `--list-active` in `validate_docs.py` is now the single resolver behind all three pickers, using the same PyYAML parse as every other check in the file.

### Added

- **`hooks/hooks.json` + `scripts/deprecated_read_guard.sh`** — the read gate described above. Scope check first (ancestor `.immutable-prd/`), cheap bash pre-filter before any python spawns, real PyYAML frontmatter parse mirroring `validate_docs.py`, successor discovery in the same domain directory (fan-out listed in full; a successor that is itself deprecated is marked, so the chain walks itself). Ships an off switch — `IMMUTABLE_DEPRECATED_GUARD_DISABLE=1` — because the plugin system has no per-hook toggle, and instrumentation via `IMMUTABLE_DEPRECATED_GUARD_LOG=<path>` (one `withheld`/`pass` line per gated Read), without which a no-op hook and an ignored emission are indistinguishable from outside. Requires `jq` (new, hook-only) and python3 + PyYAML (already required by the validator); missing PyYAML fails **open** with a deterministic warning. Hooks load at session start, so picking up the plugin version that adds this requires a session restart.
- **`scripts/deprecated_read_guard.test.sh`** — 21 hermetic unit checks: the wrong-shape regression above, both YAML directions the grep got wrong, supersede topology (no-successor, fan-out, dead successor marked), scope discipline (out-of-tree, non-markdown, frontmatter-less, scalar-frontmatter, nonexistent, relative paths all pass through untouched), and the kill switch. A `--e2e` mode loads this checkout into a real headless session via `--plugin-dir` — proving `hooks/hooks.json` discovery, `${CLAUDE_PLUGIN_ROOT}` resolution, and that the harness *accepts* the emission — and asserts on the Read **tool_result** extracted from `stream-json`, not on the final reply. The distinction is not pedantry: the banner itself sanctions deliberate raw access, so a model that has seen it may legitimately fetch and quote the body in its *reply* through a non-Read channel — one haiku run did exactly that (2026-07-20) while the Read result was correctly withheld. Reply-level assertions test model policy; tool_result assertions test the gate.
- **`--list-active` (and `--domain`) in `scripts/validate_docs.py`** — prints active (non-deprecated) docs, one `<absolute path>` TAB `<domain>` line each, sorted; `--json` emits the same as structured objects. A doc is excluded only on a *positively parsed* `deprecated: true`; one whose frontmatter is missing or malformed is listed with domain `-` and a stderr warning — fail-open, because the resolver's contract is that a live doc is never silently dropped, and that holds even under `--domain` (an unknown domain cannot be proven not to match). Passing `--domain` without `--list-active`, or the strict-body flags with it, warns rather than silently doing nothing.
- **`scripts/validate_docs_list_active.test.sh`** — 10-case regression test: exact output contract (sorted, TAB-separated, `README.md`/`TEMPLATE.md` name-excluded), both grep-wrong directions, the fail-open warning, `--type`/`--domain` scoping including the unknown-domain rule, empty-listing-exits-0, `--json` shape, the scalar-frontmatter crash fix below, and the no-effect warning.

### Changed

- **`design/SKILL.md` §1.2, `prd/SKILL.md` §1.2.4, `plan-review-ceo/SKILL.md` Phase 0.1** — all three active-pitch pickers now invoke the resolver with fixed flags (`--type pitch --list-active`, plus `--config "$IMMUTABLE_PRD_SPEC_CONFIG"` where the skill may run outside the spec repo); the `grep -L` picker and both prose-only filters are gone. `design` §1.2 also gains an explicit empty-result branch (all pitches deprecated → same `no_pitches_found` handling as an empty directory). `plan-review-ceo`'s Phase 0.1 path math for `PITCHES_DIR` — which existed only to scope the deleted prose filter — is removed; `SPEC_ROOT` stays, feeding the picker render.
- **`README.md`** — new section documenting the hook: what it withholds, why read-time, the deliberate Bash escape, the off switch, the `jq` requirement, and the session-restart note.
- **`SCHEMA.md`** — the validator paragraph now names `--list-active` as the canonical picker resolver, and the enforcement description gains the read-time layer alongside generation-time and CI, with a pointer to the README section.

### Fixed

- **`load_frontmatter` returned a scalar for scalar frontmatter, crashing the validator.** `---\njust text\n---` parses fine under `yaml.safe_load` — to a string, which `check_frontmatter` then called `.keys()` on: an `AttributeError` traceback instead of a reported violation, taking the whole run down with it. Non-mapping frontmatter now reports as missing/malformed frontmatter like any other parse failure (the read gate's own parser already handled this case; the two parsers now agree).

### Backward compatibility

- **No schema, config, or profile change; no migration.** No new config keys, no version bumps in either schema. A repo that never deprecates a doc sees no behavior change anywhere.
- **The hook is additive and carries its own off switch.** It fires only on Read-tool reads of `deprecated: true` docs under a `.immutable-prd/` ancestor; everything else passes through byte-identically. `IMMUTABLE_DEPRECATED_GUARD_DISABLE=1` restores pre-0.10.0 behavior exactly without touching the plugin.
- **Non-Read channels are intentionally out of scope.** Bash (`cat`, `git show`) — and any other tool that can read bytes — still reads raw bodies. The gate prevents *unknowing* implementation from a dead spec; it is not, and does not claim to be, an access control.
- **One new runtime dependency for the hook only**: `jq`. The validator, skills, and starters are unaffected by its absence; without it the hook cannot parse its stdin and passes reads through.
- **Starters are unchanged** and still pin `immutable--v0.8.0` in their CI workflow; the validator's new `--list-active` mode is additive and changes no validation verdict. The existing `validate_docs.test.sh` and `validate_docs_strict_since.test.sh` suites stay green.
