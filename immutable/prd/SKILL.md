...
3. Otherwise fetch through the parser — one `--binding` per `profile.requirement_contract.ticket_sections[]` entry, plus one `--optional-binding <id>` for every entry whose `required` is `false` (v0.12+; a missing `required` key means `true`, so a profile predating the field passes no `--optional-binding` at all and behaves exactly as before):
   ...
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/requirement_contract.py" fetch \
     ...
     [--optional-binding "<id of each ticket_sections entry with required: false>"] \
   ...
   **The same `required` values must reach the reconciling half.** The `coverage` and `drift` lines this run hands to the author (Stage 5, Stage 7) carry the identical `--optional-binding` flags. A section this stage waved through as optional while CI still demands it — or the reverse — is two layers disagreeing about the contract, which is the defect class the v0.12 fixes exist to close.
4. Handle the exit code:
   ...
   - `1` — a **required** binding section is missing or empty. Render `prd.contract.section_missing` with `{ticket_ref}` (`<repo>#<id>`), `{errors}` = the parser's `errors[]` one per line, and the same `{next_action}` split. A ticket without the sections the team declared mandatory cannot be contracted against; under `required` the only ways forward are fixing the ticket or declaring an exemption.
   - `0` — render `prd.contract.ticket_summary` with `{ticket_ref}`, `{title}`, `{version}`, `{read_at}` from `source`, `{section_rows}` and `{warning_count}`. `{section_rows}` is one row per binding, in the parser's order: `prd.contract.section_row` (`{heading}`, `{item_count}`, `{group_count}` = number of groups) for a binding the ticket supplies, and `prd.contract.section_row_absent` (`{heading}`, `{why}` from the matching `absent[]` row) for one it does not — an optional section that is absent is **named**, never dropped from the summary, because a section the ticket never carried is unbound work nobody has claimed. Then continue to the materials prompt; Stage 1.6 follows this stage.
